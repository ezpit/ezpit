from PySide6.QtWidgets import QDialog, QCheckBox, QPushButton, QVBoxLayout, QFileDialog, QMessageBox
from PySide6.QtCore import QSettings
from ezpit.gui.model.saver import save_selected_graphs, save_graph_data, save_calculated_graph_data
import os
...
class SaveMenu(QDialog):
    def __init__(self, parent):
        ...
        if not self._has_data(0): self.cb_iq_img.hide()
        if not self._has_data(1): self.cb_sq_img.hide()
        if not self._has_data(2): self.cb_fq_img.hide()
        if not self._has_data(3): self.cb_gr_img.hide()
    ...
    def export_images(self):
        ...
        if selected_plots:
            ...
            save_selected_graphs(
                self.parent_window, selected_checkboxes, selected_plots, folder_path,
                base_name=base_name
            )
    ...
    def export_files(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Export Error", f"An error occurred while exporting files:\n{str(e)}")
