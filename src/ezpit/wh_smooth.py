...
def smooth_whittaker(y: np.ndarray, lambda_: float = 1000.0, order: int = 2) -> np.ndarray:
...
def _linear_interp_seg(q_seg, q_full, y_full, mask):
    ...
    if y_anchor[0] > 0 and y_anchor[1] > 0:
        ...
        log_interp = np.interp(q_seg, q_anchor, [np.log10(y_anchor[0]), np.log10(y_anchor[1])])
...
def _spline_interp_seg(q_seg, q_full, y_full, mask, anchor_pts=10):
    ...
    if np.all(y_anc > 0):
        cs = CubicSpline(q_anc, np.log10(y_anc), bc_type="natural", extrapolate=True)
...
def _manual_spline_seg(q_seg, manual_pts):
    ...
    if np.all(y_pts > 0):
        cs = CubicSpline(q_pts, np.log10(y_pts), bc_type="natural", extrapolate=True)
        return 10.0 ** cs(q_seg)
    else:
...
def range_smooth(q: np.ndarray, y: np.ndarray, ranges: list) -> np.ndarray:
    """
...
def load_file(path: str):
    ...
    if len(x_vals) < 2:
        raise ValueError("No two-column numeric data found in the file. Expected two columns (x and intensity).")
...
class QRangeRow(QWidget):
    ...
    def __init__(self, default_lambda=1000, default_order=2, parent=None):
        ...
        self.manual_btn.setStyleSheet("QPushButton:checked { background:#E84855; color:white; font-weight:bold; }")
    ...
    def get_range(self):
        ...
        try:
            ...
            d = {"q0": q0, "q1": q1, "mode": mode, "blend": self.blend_spin.value()}
            if mode == "wh":
...
class WHRangeSmoother(QMainWindow):
    ...
    def _show_about_dialog(self):
        ...
        if os.path.exists(logo_path):
            ...
            if not pix.isNull():
                box.setIconPixmap(pix.scaledToWidth(220, Qt.TransformationMode.SmoothTransformation))
        box.exec()
    ...
    def _build_ui(self):
        ...
        for label in ("I(q)", "S(q)", "F(q)"):
            ...
            rb.toggled.connect(lambda checked, lbl=label: self._on_type(lbl, checked))
        ...
        # Crosshair lines
        self._vline_main = pg.InfiniteLine(
            angle=90,
            movable=False,
            pen=pg.mkPen("#AAAAAA", width=1, style=Qt.PenStyle.DashLine),
        )
        self._hline_main = pg.InfiniteLine(
            angle=0,
            movable=False,
            pen=pg.mkPen("#AAAAAA", width=1, style=Qt.PenStyle.DashLine),
        )
        ...
        self.plot_main.scene().sigMouseMoved.connect(lambda pos: self._on_mouse_move(pos, "main"))
        self.plot_main.scene().sigMouseClicked.connect(self._on_plot_clicked)
        ...
        self._vline_diff = pg.InfiniteLine(
            angle=90,
            movable=False,
            pen=pg.mkPen("#AAAAAA", width=1, style=Qt.PenStyle.DashLine),
        )
        self._hline_diff = pg.InfiniteLine(
            angle=0,
            movable=False,
            pen=pg.mkPen("#AAAAAA", width=1, style=Qt.PenStyle.DashLine),
        )
        ...
        self.plot_diff.scene().sigMouseMoved.connect(lambda pos: self._on_mouse_move(pos, "diff"))
        ...
        for pw, name in ((self.plot_main, "main"), (self.plot_diff, "diff")):
            ...
            vb.mouseDoubleClickEvent = lambda e, v=vb, n=name: self._on_plot_double_click(e, v, n)
        ...
        self._mouse_hint = QLabel(
            "🖱  Drag: Zoom-in   |   Double-click: Reset   |   Right-drag: Zoom   |   Z: Pan/Zoom   R: Reset"
        )
    ...
    def _pick_bkg(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            self._last_dir,
            "Data Files (*.chi *.iq *.sq *.fq *.xy *.gr *.txt *.dat *.csv *.tsv);;All Files (*)",
        )
        if not path:
            return
    ...
    def _subtract_bkg(self):
        ...
        self._apply_log_scale()
        self._update_main_plot()
        self.statusBar().showMessage(f"Background subtracted  |  alpha = {alpha}  |  Bkg: {os.path.basename(bkg_path)}")
    ...
    def _pick_reference(self):
        ...
        self.statusBar().showMessage(
            f"Reference loaded: {os.path.basename(path)}  ({len(q_r)} pts, q = {q_r[0]:.3f}–{q_r[-1]:.3f})"
        )
    ...
    def _pick_file(self):
        ...
        self._update_main_plot()
        self.statusBar().showMessage(
            f"Loaded: {os.path.basename(path)}  |  {len(q)} points,  q = {q[0]:.3f} ~ {q[-1]:.3f} Å⁻¹"
        )
    ...
    def _apply_smoothing(self):
        ...
        if not ranges:
            try:
                ...
                QMessageBox.warning(self, "Bad Parameter", "Section 4 default lambda is invalid.")
                return
        ...
        n_pts = sum(((self.q >= r["q0"]) & (self.q <= r["q1"])).sum() for r in ranges)

        def _r_summary(r):
            m = r["mode"]
            if m == "wh":
                return f"q[{r['q0']}–{r['q1']}] WH λ={r['lambda']:g} ord={r['order']} blend={r['blend']}"
            elif m == "linear":
                return f"q[{r['q0']}–{r['q1']}] Linear blend={r['blend']}"
            elif m == "spline":
                return f"q[{r['q0']}–{r['q1']}] Spline anc={r.get('anchor_pts', '?')} blend={r['blend']}"
            elif m == "manual":
        ...
        if full_range_mode:
            ...
            self.statusBar().showMessage(
                f"Processed {n_pts} pts across {len(ranges)} range(s):  " + "  |  ".join(_r_summary(r) for r in ranges)
            )

    def _save_result(self):
        if self.y_smoothed is None:
            QMessageBox.warning(self, "No Result", "Apply smoothing first.")
            return

        src = self.file_edit.text()
        base = os.path.splitext(os.path.basename(src))[0]
        default = os.path.join(self._last_dir, f"{base}_wh_smoothed.chi")
        ...
        def _r_str(r):
            m = r["mode"]
            if m == "wh":
                return f"q[{r['q0']}-{r['q1']}] WH lambda={r['lambda']:g} order={r['order']} blend={r['blend']}"
            elif m == "linear":
                return f"q[{r['q0']}-{r['q1']}] Linear blend={r['blend']}"
            elif m == "spline":
                return f"q[{r['q0']}-{r['q1']}] Spline anchor_pts={r.get('anchor_pts', '?')} blend={r['blend']}"
            elif m == "manual":
                n = len(r.get("manual_pts", []))
                return f"q[{r['q0']}-{r['q1']}] Manual({n} pts) blend={r['blend']}"
            else:
    ...
    def _add_range_row(self):
        ...
        row.manual_btn.toggled.connect(lambda checked, r=row: self._on_manual_btn_toggled(r, checked))
    ...
    def _on_manual_btn_toggled(self, row, checked):
        ...
        if checked:
            ...
            self.statusBar().showMessage("Manual Spline: LEFT-CLICK on graph to add points  |  RIGHT-CLICK to finish")
        else:
            ...
            self.statusBar().showMessage(
                f"Manual editing stopped  ({len(row._manual_pts)} pts stored)"  # noqa: SLF001
            )
    ...
    def _on_plot_clicked(self, event):
        ...
        n = len(self._active_row._manual_pts)  # noqa: SLF001
        self._update_main_plot()
        self.statusBar().showMessage(f"Added: q={q_c:.4f}, y={y_c:.4g}  ({n} pts)  |  Right-click to finish")
        event.accept()
    ...
    def _update_main_plot(self):
        ...
        if self._legend_pos is None:
            ...
            try:
                self._legend.anchor(itemPos=(1, 0), parentPos=(1, 0), offset=(-10, 10))
            except Exception:
                pass
        else:
        ...
        for r in self._get_ranges():
            ...
            lr = pg.LinearRegionItem([r["q0"], r["q1"]], movable=False, brush=brush)
            lr.setZValue(-10)
        ...
        use_log = self._data_type == "I(q)" and hasattr(self, "log_chk") and self.log_chk.isChecked()
        for i, row in enumerate(self._get_rows()):
            if row.mode_combo.currentText() != "Manual Spline":
                continue
            pts = row._manual_pts  # noqa: SLF001
            if not pts:
                continue
    ...
    def _update_diff_plot(self):
        ...
        self.plot_diff.plot(self.q, diff, pen=pg.mkPen("#1A8A4A", width=1.8))
        self.plot_diff.addLine(y=0, pen=pg.mkPen("#888888", style=Qt.PenStyle.DashLine))
...
def main():
    ...
    win._fit_and_center(1200, 760)  # noqa: SLF001
    win.show()
    sys.exit(app.exec())
